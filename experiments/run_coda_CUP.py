...
from coda_scheduler_CUP import CODAScheduler
...
# Final O2I/CUP settings.
CUP_REFERENCE_SIZE = 64
CUP_REFERENCE_SEED = 2026
...
class CODACallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result: dict, **kwargs):
        ...
        # ---------------------------------------------------------------------
        # gp_uncertainty is now CUP in [0, 1].
        custom["coda_gp_uncertainty"] = self._safe_float(
            bridge.get("gp_uncertainty", 0.0), 0.0
        )
        custom["coda_gp_uncertainty_raw"] = self._safe_float(
            bridge.get("gp_uncertainty_raw", np.nan), np.nan
        )
        custom["coda_gp_kernel_variance"] = self._safe_float(
            bridge.get("gp_kernel_variance", np.nan), np.nan
        )
        custom["coda_gp_reference_uncertainty_median"] = self._safe_float(
            bridge.get("gp_reference_uncertainty_median", np.nan), np.nan
        )
        custom["coda_gp_reference_uncertainty_iqr"] = self._safe_float(
            bridge.get("gp_reference_uncertainty_iqr", np.nan), np.nan
        )
        ...
        if self._iter_count % 5 == 0:
            try:
                ...
                cup = custom["coda_gp_uncertainty"]
                logger.info(
...
def _save_metadata(
    ...
    *,
    ...
    payload = {
        ...
        "max_gp_points": 1000,
        "o2i_uncertainty": {
            "method": "contextual_uncertainty_percentile",
            "posterior_model": "observation_based_gp",
            "reference_size": CUP_REFERENCE_SIZE,
            "reference_seed": CUP_REFERENCE_SEED,
            "reference_design": "fixed_uniform_normalized_search_domain",
            "percentile": "midrank_empirical",
            "range": [0.0, 1.0],
        },
...
def _extract_metrics(
    ...
    columns_wanted = [
        ...
        "custom_metrics/coda_gp_kernel_variance",
        "custom_metrics/coda_gp_reference_uncertainty_median",
        "custom_metrics/coda_gp_reference_uncertainty_iqr",
...
def run_experiment(
    ...
    scheduler = CODAScheduler(
        ...
        entropy_guard=ENTROPY_GUARD,
        reward_z_clip=4.0,
        uncertainty_reference_size=CUP_REFERENCE_SIZE,
        uncertainty_reference_seed=CUP_REFERENCE_SEED,
        synch=False,
    )
