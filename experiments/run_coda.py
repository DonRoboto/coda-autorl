...
from coda_scheduler import CODAScheduler
...
# Explicit environment-specific performance bounds used only for contextual GP
# normalization.  They are never inferred from post-hoc experimental results.
DEFAULT_REWARD_CONTEXT_BOUNDS: Dict[str, Tuple[float, float]] = {
    "HalfCheetah-v5": (-500.0, 1500.0),
    "Walker2d-v5": (-50.0, 1000.0),
    "Hopper-v5": (-50.0, 1500.0),
    "Swimmer-v5": (-50.0, 500.0),
    "Ant-v5": (-150.0, 400.0),
    "Humanoid-v5": (50.0, 500.0),
}
...
def _get_reward_bounds(env_name: str, w_pb2_params: dict) -> Tuple[float, float]:
    override = w_pb2_params.get("reward_context_bounds")
    if override is not None:
        if len(override) != 2:
            raise ValueError("reward_context_bounds must be [low, high]")
        return float(override[0]), float(override[1])

    if env_name not in DEFAULT_REWARD_CONTEXT_BOUNDS:
        raise KeyError(
            f"No predefined reward context bounds for {env_name}. "
            "Add them explicitly or provide w_pb2_params['reward_context_bounds']."
        )
    return DEFAULT_REWARD_CONTEXT_BOUNDS[env_name]
...
def _save_metadata(
    ...
    *,
    ...
    payload = {
        ...
        "reward_context_bounds": list(map(float, reward_bounds)),
...
def run_experiment(env_name: str, seed: int, w_pb2_params: dict) -> bool:
    ...
    reward_bounds = _get_reward_bounds(env_name, w_pb2_params)

    context_bounds = {
        "T_before": (0.0, float(TIMESTEPS_MAX)),
        "R_before": reward_bounds,
        "S_before": (0.0, 1.0),
    }
    ...
    try:
        ...
        # Apply the explicit stopping condition logic as a config flag 
        # to circumvent deprecated run_config structures
        tuner = tune.Tuner(
            "PPO",
            ...
            #     stop={
            #         "timesteps_total": TIMESTEPS_MAX,
            #         },
            #     checkpoint_config=tune.CheckpointConfig(
            #         # Removed deprecated checkpoint_at_end=True
            #         num_to_keep=4,
            #     ),
            # ),
            run_config=tune.RunConfig(
