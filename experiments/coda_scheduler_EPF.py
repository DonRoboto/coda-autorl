...
---------------------
- ``full``: CODA (I2O contextual surrogate + O2I exploration-pressure feedback)
- ``i2o``:  CODA-I2O (contextual surrogate only)
- ``o2i``:  CODA-O2I (PB2-style surrogate + O2I exploration-pressure feedback)

Final O2I design
----------------
CODA communicates the *exploration pressure* of the outer PB2 decision to the
inner PPO learner. After PB2 selects a configuration with the UCB criterion

    acquisition = posterior_mean + kappa * batch_adjusted_variance,

the O2I signal is the relative contribution of the exploration bonus:

    pressure = exploration_bonus /
               (abs(posterior_mean) + exploration_bonus + epsilon).

The pressure lies in [0, 1] and directly scales a bounded PPO entropy increment.
This reuses quantities already computed by PB2 and adds essentially no extra GP
cost.
...
8. Use the batch-adjusted GP variance exactly as it appears in the PB2 UCB
   acquisition, and communicate its relative exploration contribution to PPO.
"""
...
class CODAScheduler(PopulationBasedTraining):
    ...
    def _apply_o2i_feedback(
        ...
        *,
        ...
        """Use outer-loop exploration pressure to modulate PPO entropy."""
        out = deepcopy(config)
        ...
        if guided_update and self.use_o2i_feedback:
            ...
            increment = float(
                ...
                * np.clip(exploration_pressure, 0.0, 1.0)
            )
            increment = float(
