...
---------------------
- ``full``: CODA (I2O contextual surrogate + O2I health-gated exploration-pressure feedback)
- ``i2o``:  CODA-I2O (contextual surrogate only)
- ``o2i``:  CODA-O2I (PB2-style surrogate + O2I health-gated exploration-pressure feedback)

Final O2I design
----------------
CODA first measures the *exploration pressure* of the outer PB2 decision:

    pressure = exploration_bonus /
               (abs(posterior_mean) + exploration_bonus + epsilon).

It then gates that outer-loop pressure by the donor learner's pre-decision
health diagnostic S in [0, 1]:

    gated_signal = pressure * learner_readiness.

The gated signal controls the PPO entropy increment:

    entropy_increment = o2i_entropy_scale * gated_signal.

Thus, strong outer-loop exploration increases inner-loop exploration only when
the learner is in a sufficiently healthy state to absorb it.  All quantities are
pre-decision and the design adds no extra GP fit or prediction.
...
8. Use the batch-adjusted GP variance exactly as it appears in the PB2 UCB
   acquisition to quantify outer-loop exploration pressure.
9. Gate that pressure by the donor learner-state diagnostic before modulating
   PPO entropy.
"""
...
def _select_config(
    *,
    ...
    context_bounds: Dict[str, Tuple[float, float]],
    reward_z_clip: float = 4.0,
    compute_o2i_feedback: bool = True,
    o2i_signal_epsilon: float = 1e-8,
) -> Tuple[
    np.ndarray,
    float,
    float,
    ...
    """Fit TV-GPs and return PB2 proposal plus O2I audit quantities.

    The PB2 proposal is unchanged.  The O2I signal measures how strongly the
    selected UCB value is driven by the exploration bonus relative to the
    magnitude of the observation-based posterior mean.
    """
    ...
    # Observation-based GP: fitted only to real observed transitions.
    ...
    # -------------------------------------------------------------------------
    # O2I: outer-loop exploration-pressure feedback
    # -------------------------------------------------------------------------
    if compute_o2i_feedback:
        x_selected = np.concatenate((fixed, xt_norm)).reshape(1, -1)

        # Exploitation component of the PB2 UCB decision.
        selected_mean = float(
            mean_model.predict(x_selected).reshape(-1)[0]
        )

        # Exploration component uses the SAME batch-adjusted variance used by
        # the PB2 acquisition function.
        _, selected_std = variance_model.predict(
            x_selected,
            return_std=True,
        )
        selected_variance = float(selected_std.reshape(-1)[0] ** 2)

        n_obs = int(
            getattr(mean_model, "X_train_", np.empty((0,))).shape[0]
        )
        beta_t = float(
            0.2 + max(0.0, np.log(0.4 * max(n_obs, 1)))
        )
        kappa_t = float(np.sqrt(beta_t))

        exploration_bonus = float(
            kappa_t * max(0.0, selected_variance)
        )
        acquisition_value = float(
            selected_mean + exploration_bonus
        )

        # Fraction of decision magnitude attributable to exploration.
        denominator = (
            abs(selected_mean)
            + exploration_bonus
            + float(o2i_signal_epsilon)
        )
        exploration_pressure = float(
            np.clip(
                exploration_bonus / denominator,
                0.0,
                1.0,
            )
        )
...
class CODAScheduler(PopulationBasedTraining):
    ...
    def __init__(
        ...
        *,
        ...
        self.o2i_signal_epsilon = float(o2i_signal_epsilon)
        ...
        if self.o2i_signal_epsilon <= 0:
            raise ValueError("o2i_signal_epsilon must be > 0")
    ...
    def on_trial_add(
        ...
        bridge.setdefault("o2i_exploration_pressure", 0.0)
        bridge.setdefault("o2i_learner_readiness", 0.0)
        bridge.setdefault("o2i_health_gated_signal", 0.0)
        bridge.setdefault("gp_selected_mean", np.nan)
        bridge.setdefault("gp_selected_variance", np.nan)
        bridge.setdefault("gp_exploration_bonus", np.nan)
        bridge.setdefault("gp_ucb_beta", np.nan)
        bridge.setdefault("gp_ucb_kappa", np.nan)
        bridge.setdefault("gp_acquisition_value", np.nan)
    ...
    def _save_trial_state(
        ...
        row = {
            ...
            **dict(zip(hp_names, hp_values)),
            ...
            "o2i_exploration_pressure": _safe_float(
                bridge.get("o2i_exploration_pressure", 0.0), 0.0
            ),
            "o2i_learner_readiness": _safe_float(
                bridge.get("o2i_learner_readiness", 0.0), 0.0
            ),
            "o2i_health_gated_signal": _safe_float(
                bridge.get("o2i_health_gated_signal", 0.0), 0.0
            ),
            "gp_selected_mean": _safe_float(
                bridge.get("gp_selected_mean", np.nan), np.nan
            ),
            "gp_selected_variance": _safe_float(
                bridge.get("gp_selected_variance", np.nan), np.nan
            ),
            "gp_exploration_bonus": _safe_float(
                bridge.get("gp_exploration_bonus", np.nan), np.nan
            ),
            "gp_ucb_beta": _safe_float(
                bridge.get("gp_ucb_beta", np.nan), np.nan
            ),
            "gp_ucb_kappa": _safe_float(
                bridge.get("gp_ucb_kappa", np.nan), np.nan
            ),
            "gp_acquisition_value": _safe_float(
                bridge.get("gp_acquisition_value", np.nan), np.nan
            ),
    ...
    def _append_lineage_anchor(
        ...
        *,
        ...
        row = {
            ...
            "is_synthetic": True,
            "guided_update": False,
            "o2i_exploration_pressure": 0.0,
            "o2i_learner_readiness": 0.0,
            "o2i_health_gated_signal": 0.0,
            "gp_selected_mean": np.nan,
            "gp_selected_variance": np.nan,
            "gp_exploration_bonus": np.nan,
            "gp_ucb_beta": np.nan,
            "gp_ucb_kappa": np.nan,
            "gp_acquisition_value": np.nan,
    ...
    @staticmethod
    def _learner_readiness_from_row(
        donor_row: Optional[pd.Series],
    ) -> float:
        """Return the donor's valid pre-decision learner health in [0, 1].

        Invalid or unavailable diagnostics close the O2I gate (readiness=0),
        preventing an uncertainty/exploration-driven entropy increase when the
        learner health signal cannot be trusted.
        """
        if donor_row is None:
            return 0.0

        valid = _safe_float(
            donor_row.get("stability_valid", 0.0),
            0.0,
        ) >= 0.5
        if not valid:
            return 0.0
        ...
        readiness = _safe_float(
            donor_row.get("stability_index", np.nan),
            np.nan,
        )
        if not np.isfinite(readiness):
            return 0.0
    ...
    def _propose_from_gp(
        ...
        fallback = (
            ...
            False,
            0.0,     # exploration pressure
            np.nan,  # selected mean
            np.nan,  # selected variance
            np.nan,  # exploration bonus
            np.nan,  # beta
            np.nan,  # kappa
            np.nan,  # acquisition value
        )
        ...
        (
            ...
            compute_o2i_feedback=self.use_o2i_feedback,
            o2i_signal_epsilon=self.o2i_signal_epsilon,
        )
        ...
        return (
            new_flat,
            True,
            float(np.clip(exploration_pressure, 0.0, 1.0)),
            float(selected_mean) if np.isfinite(selected_mean) else np.nan,
            float(selected_variance) if np.isfinite(selected_variance) else np.nan,
            float(exploration_bonus) if np.isfinite(exploration_bonus) else np.nan,
            float(beta_t) if np.isfinite(beta_t) else np.nan,
            float(kappa_t) if np.isfinite(kappa_t) else np.nan,
            float(acquisition_value) if np.isfinite(acquisition_value) else np.nan,
        )
    ...
    def _apply_o2i_feedback(
        ...
        *,
        ...
        """Gate outer-loop exploration pressure by learner health.

        The effective O2I signal is

            health_gated_signal = exploration_pressure * learner_readiness.

        Both terms are pre-decision quantities in [0, 1].
        """
        out = deepcopy(config)
        bridge = self._bridge(out)

        bridge["guided_update"] = bool(guided_update)
        bridge["o2i_exploration_pressure"] = float(
            exploration_pressure if guided_update else 0.0
        )
        bridge["o2i_learner_readiness"] = float(
            learner_readiness if guided_update else 0.0
        )
        bridge["o2i_health_gated_signal"] = float(
            health_gated_signal if guided_update else 0.0
        )
        bridge["gp_selected_mean"] = (
            float(selected_mean)
            if guided_update and np.isfinite(selected_mean)
            else np.nan
        )
        bridge["gp_selected_variance"] = (
            float(selected_variance)
            if guided_update and np.isfinite(selected_variance)
            else np.nan
        )
        bridge["gp_exploration_bonus"] = (
            float(exploration_bonus)
            if guided_update and np.isfinite(exploration_bonus)
            else np.nan
        )
        bridge["gp_ucb_beta"] = (
            float(beta_t)
            if guided_update and np.isfinite(beta_t)
            else np.nan
        )
        bridge["gp_ucb_kappa"] = (
            float(kappa_t)
            if guided_update and np.isfinite(kappa_t)
            else np.nan
        )
        bridge["gp_acquisition_value"] = (
        ...
        if guided_update and self.use_o2i_feedback:
            ...
            increment = float(
                ...
                * np.clip(health_gated_signal, 0.0, 1.0)
            )
            increment = float(
    ...
    def _get_new_config(
        ...
        # Health-gated O2I feedback.  The learner-state signal is taken from
        # the donor's latest real pre-decision observation, so no future
        # information from the receiver is used.
        learner_readiness = (
            self._learner_readiness_from_row(donor_row)
            if guided and self.use_o2i_feedback
            else 0.0
        )
        health_gated_signal = float(
            np.clip(
                exploration_pressure * learner_readiness,
                0.0,
                1.0,
            )
        )
        ...
        new_config = self._apply_o2i_feedback(
            ...
            health_gated_signal=health_gated_signal,
