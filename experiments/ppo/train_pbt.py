#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PBT + PPO baseline using the shared repository configuration."""
...
PBT_SMOKE_TEST = os.environ.get("PBT_PPO_SMOKE", "1") != "0"
PBT_SMOKE_ENV = os.environ.get("PBT_PPO_SMOKE_ENV", "Hopper-v5")
PBT_SMOKE_SEEDS = [
    int(os.environ.get("PBT_PPO_SMOKE_SEED", str(SMOKE_TRAINING_SEED)))
]
PBT_SMOKE_STEPS = int(os.environ.get("PBT_PPO_SMOKE_STEPS", "500000"))
...
def _validate_result_grid(result_grid, *, target_steps: int) -> None:
    """Reject incomplete or restore-invalid PBT-PPO population runs."""
    ...
    mismatch_col = "custom_metrics/pbt_effective_hp_mismatch_count"
    ...
    for idx, result in enumerate(results):
        ...
        if mismatch_col in df.columns:
            values = pd.to_numeric(df[mismatch_col], errors="coerce")
            finite = values[np.isfinite(values)]
    ...
    if problems:
        raise RuntimeError(
            "Invalid PBT-PPO population run:\n  - " + "\n  - ".join(problems)
        )
...
def run_experiment(
    ...
    hyperparam_mutations = tune_search_space()
...
def main() -> None:
    ...
    if PBT_SMOKE_TEST:
        if PBT_SMOKE_ENV not in ENVIRONMENTS:
            raise KeyError(f"Unknown smoke environment: {PBT_SMOKE_ENV}")
        experiment_items = [
            (
                PBT_SMOKE_ENV,
                {
                    **ENVIRONMENT_CONFIG[PBT_SMOKE_ENV],
                    "semillas": PBT_SMOKE_SEEDS,
                },
            )
        ]
        target_steps = PBT_SMOKE_STEPS
    else:
    ...
    for env_name, env_cfg in experiment_items:
        for seed in env_cfg["semillas"]:
            # For smoke mode only, temporarily override the global stop via
            # a shallow config passed to run_experiment.
            original_max = globals()["TIMESTEPS_MAX"]
            try:
                globals()["TIMESTEPS_MAX"] = int(target_steps)
                ok = run_experiment(env_name, int(seed), env_cfg)
            finally:
