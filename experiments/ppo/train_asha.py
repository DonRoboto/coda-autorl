...
from configs.ppo_config import (  # noqa: E402
    ...
    ASHA_BRACKETS,
    ASHA_BUDGET_EXCELLENT_LOW,
    ASHA_BUDGET_EXCELLENT_HIGH,
    ASHA_BUDGET_ACCEPTABLE_LOW,
    ASHA_BUDGET_ACCEPTABLE_HIGH,
    HYPERPARAM_BOUNDS,
    FIXED_VF_LOSS_COEFF,
    BASE_ENTROPY_COEFF,
    FIXED_GAMMA,
...
ASHA_TARGET_AGGREGATE_BUDGET = NOMINAL_AGGREGATE_BUDGET
FIXED_ENTROPY_COEFF = BASE_ENTROPY_COEFF

RESULTS_ROOT = REPO_ROOT / "results"

ENVIRONMENT_CONFIG = {
    env_name: {"semillas": list(TRAINING_SEEDS)}
    for env_name in ENVIRONMENTS
}

ASHA_SMOKE_TEST = os.environ.get("ASHA_PPO_SMOKE", "1") != "0"
ASHA_SMOKE_ENV = os.environ.get("ASHA_PPO_SMOKE_ENV", "Hopper-v5")
ASHA_SMOKE_SEEDS = [
    int(os.environ.get("ASHA_PPO_SMOKE_SEED", str(SMOKE_TRAINING_SEED)))
]
ASHA_SMOKE_STEPS = int(os.environ.get("ASHA_PPO_SMOKE_STEPS", "500000"))
...
def main() -> None:
    ...
    if ASHA_SMOKE_TEST:
        if ASHA_SMOKE_ENV not in ENVIRONMENTS:
            raise KeyError(f"Unknown smoke environment: {ASHA_SMOKE_ENV}")
        experiment_items = [
            (
                ASHA_SMOKE_ENV,
                {
                    "semillas": ASHA_SMOKE_SEEDS,
                },
            )
        ]
        target_steps = ASHA_SMOKE_STEPS
    else:
