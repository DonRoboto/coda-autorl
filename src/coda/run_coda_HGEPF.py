...
from coda_scheduler_HGEPF import CODAScheduler
...
# Health-gated O2I exploration-pressure settings.
O2I_ENTROPY_SCALE = 0.005
O2I_MAX_INCREMENT = 0.005
O2I_SIGNAL_EPSILON = 1e-8
ENTROPY_GUARD = 0.05

# -----------------------------------------------------------------------------
# Hopper smoke-test controls
# -----------------------------------------------------------------------------
# Leave True for the validation run. Set False before the final campaign.
...
class CODACallback(DefaultCallbacks):
    ...
    def on_train_result(self, *, algorithm, result: dict, **kwargs):
        ...
        # ---------------------------------------------------------------------
        custom["coda_o2i_exploration_pressure"] = self._safe_float(
            bridge.get("o2i_exploration_pressure", 0.0), 0.0
        )
        custom["coda_o2i_learner_readiness"] = self._safe_float(
            bridge.get("o2i_learner_readiness", 0.0), 0.0
        )
        custom["coda_o2i_health_gated_signal"] = self._safe_float(
            bridge.get("o2i_health_gated_signal", 0.0), 0.0
        )
        custom["coda_gp_selected_mean"] = self._safe_float(
            bridge.get("gp_selected_mean", np.nan), np.nan
        )
        custom["coda_gp_selected_variance"] = self._safe_float(
            bridge.get("gp_selected_variance", np.nan), np.nan
        )
        custom["coda_gp_exploration_bonus"] = self._safe_float(
            bridge.get("gp_exploration_bonus", np.nan), np.nan
        )
        custom["coda_gp_ucb_beta"] = self._safe_float(
            bridge.get("gp_ucb_beta", np.nan), np.nan
        )
        custom["coda_gp_ucb_kappa"] = self._safe_float(
            bridge.get("gp_ucb_kappa", np.nan), np.nan
        )
        custom["coda_gp_acquisition_value"] = self._safe_float(
        ...
        if self._iter_count % 5 == 0:
            try:
                ...
                pressure = custom["coda_o2i_exploration_pressure"]
                readiness = custom["coda_o2i_learner_readiness"]
                gated = custom["coda_o2i_health_gated_signal"]
                logger.info(
                    "PPO diagnostics | KL=%.5f entropy=%.4f coeff=%.5f "
                    "S=%.4f pressure=%.3f readiness=%.3f gated=%.3f",
                    policy_kl,
                    policy_entropy,
                    entropy_coeff,
                    stability_index,
...
def _save_metadata(
    ...
    *,
    ...
    payload = {
        ...
        "max_gp_points": 1000,
        "o2i_feedback": {
            "method": "health_gated_ucb_exploration_pressure",
            "mean_model": "observation_based_gp",
            "variance_model": "pb2_batch_adjusted_gp",
            "exploration_pressure_formula":
                "bonus/(abs(mean)+bonus+epsilon)",
            "bonus": "kappa*predictive_variance",
            "learner_readiness": "pre-decision stability_index",
            "health_gate_formula":
                "exploration_pressure*learner_readiness",
            "signal_range": [0.0, 1.0],
            "signal_epsilon": O2I_SIGNAL_EPSILON,
        },
        ...
        "effective_max_applied_entropy": (
            0.015 if VARIANT in {"full", "o2i"} else 0.01
        ),
        "versions": {
            "python": sys.version,
            "ray": ray.__version__,
...
def _extract_metrics(
    ...
    columns_wanted = [
        ...
        "custom_metrics/coda_o2i_exploration_pressure",
        "custom_metrics/coda_o2i_learner_readiness",
        "custom_metrics/coda_o2i_health_gated_signal",
        "custom_metrics/coda_gp_selected_mean",
        "custom_metrics/coda_gp_selected_variance",
        "custom_metrics/coda_gp_exploration_bonus",
        "custom_metrics/coda_gp_ucb_beta",
        "custom_metrics/coda_gp_ucb_kappa",
        "custom_metrics/coda_gp_acquisition_value",
...
def run_experiment(
    ...
    print("\n" + "=" * 72)
    print(
        f"Starting {ALGO_NAME} | env={env_name} | "
    ...
    scheduler = CODAScheduler(
        ...
        entropy_guard=ENTROPY_GUARD,
        reward_z_clip=4.0,
        o2i_signal_epsilon=O2I_SIGNAL_EPSILON,
        synch=False,
    )
    ...
    try:
        ...
        storage_root = Path(
            f"./results/ray_tune_logs/{ALGO_NAME}"
        ).resolve()
