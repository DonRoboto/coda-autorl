...
4. Normalize reward context causally with a task-agnostic robust transform; use fixed bounds only for training progress and learner-state health.
...
def _robust_reward_to_unit_interval(
    ...
    *,
    ...
    The location/scale are estimated ONLY from reward observations already
    present in the GP training set.  A robust z-score is built from the median
    and an IQR-derived scale; MAD and standard deviation are used only as
    degenerate fallbacks.  Scores are clipped to +/- ``z_clip`` and mapped to
    [0, 1], keeping the reward coordinate commensurate with the other normalized
    GP inputs while preventing extreme returns from dominating kernel distance.

    The query reward is transformed with the training-set statistics and never
    contributes to their estimation, so the preprocessing remains strictly
    pre-decision/causal.
    """
    rewards = np.asarray(train_rewards, dtype=np.float64).reshape(-1)
    rewards = rewards[np.isfinite(rewards)]
    if rewards.size == 0:
        raise ValueError("Cannot normalize reward context without finite training rewards")
    ...
    # Convert IQR to a robust sigma-like scale under an approximately Gaussian
    # distribution.  This is only a scale convention; the subsequent clipping
    # makes the transform robust to non-Gaussian return distributions.
    ...
    # If all observed rewards are effectively identical, reward carries no
    # contextual information yet; map the entire coordinate to the midpoint.
    if not np.isfinite(scale) or scale < eps:
        return np.full(rewards.shape, 0.5, dtype=np.float64), 0.5

    train_z = np.clip((rewards - center) / scale, -z_clip, z_clip)
    query_z = float(np.clip((float(query_reward) - center) / scale, -z_clip, z_clip))
...
def _normalize_context(
    ...
    *,
    ...
    ``T_before`` and ``S_before`` use fixed known bounds. ``R_before`` uses the
    causal robust transform above, eliminating environment-specific reward
    bounds and post-hoc range tuning.
    """
    ...
    for j, name in enumerate(context_columns):
        ...
        if name not in context_bounds:
            raise ValueError(f"Missing fixed normalization bounds for context {name!r}")

        limits = np.asarray(context_bounds[name], dtype=np.float64).reshape(2, 1)
        X_norm[:, j] = _normalize(X_context_raw[:, j].reshape(-1, 1), limits).reshape(-1)
        fixed_norm[j] = float(
            _normalize(np.asarray([[new_context_raw[j]]], dtype=np.float64), limits)[0, 0]
        )
...
def _contextual_uncertainty_percentile(
    ...
    *,
    ...
    """Mid-rank empirical percentile of selected posterior uncertainty.

    Returns a value in [0, 1].

    0.0 -> selected point is among the least uncertain.
    0.5 -> selected point has typical uncertainty.
    1.0 -> selected point is among the most uncertain.

    Numerical ties receive half weight, so if all uncertainties are
    effectively identical the returned value is 0.5 instead of 1.0.
    """
...
def _select_config(
    *,
    ...
    if Xraw.ndim != 2 or Xraw.shape[1] != n_context + model_vals.shape[1]:
        raise ValueError("Xraw shape is inconsistent with context/hyperparameter dimensions")
    ...
    # ============================================================
    # IMPORTANT:
    # The acquisition still uses variance_model for PB2-style
    # batch diversification, but CODA's O2I uncertainty is evaluated
    # with mean_model, which is fitted only to real observed transitions.
    # ============================================================
    ...
    # Raw posterior standard deviation from the observation-based GP
    _, std = mean_model.predict(
        x_eval,
        return_std=True,
    )
    ...
    # Learned kernel variance of the observation-based GP
    kernel_variance = float(
        mean_model.kernel_.variance
    )
    
    # Convert kernel variance to its corresponding prior standard deviation
    prior_std = float(
        np.sqrt(
            max(kernel_variance, 1e-12)
        )
    )
    
    # Relative uncertainty used by CODA for O2I
    uncertainty = float(
        np.clip(
            uncertainty_raw / prior_std,
            0.0,
            1.0,
        )
    )
...
class CODAScheduler(PopulationBasedTraining):
    ...
    def __init__(
        ...
        *,
        ...
        # Reward is normalized causally from past observations and therefore does
        # not require task-specific fixed bounds. Only inherently bounded/fixed
        # context coordinates are validated here.
    ...
    def _context_limits(self) -> np.ndarray:
        """Return fixed context limits for debugging/backward compatibility.

        ``R_before`` is intentionally omitted from fixed bounds because it is
        causally robust-normalized at every GP fit.
        """
        names = [name for name in self._context_columns() if name != "R_before"]
        return np.asarray([self._context_bounds[name] for name in names], dtype=np.float64).T
    ...
    def on_trial_add(self, tune_controller: "TuneController", trial: Trial):
        ...
        bridge.setdefault("gp_kernel_variance", 0.0)
    ...
    def _save_trial_state(
        ...
        row = {
            ...
            **dict(zip(hp_names, hp_values)),
            ...
            "gp_uncertainty_raw": _safe_float(bridge.get("gp_uncertainty_raw", 0.0), 0.0),
            "gp_kernel_variance": _safe_float(bridge.get("gp_kernel_variance", 0.0), 0.0),
    ...
    def _latest_trial_row(self, trial: Trial) -> Optional[pd.Series]:
        ...
        if rows.empty:
            return None
        rows = rows.reset_index(drop=False).rename(columns={"index": "_insertion_order"})
        rows = rows.sort_values(["_insertion_order"], kind="mergesort")
        return rows.iloc[-1]
    ...
    def _propose_from_gp(
    ...
    ) -> Tuple[Dict, bool, float, float, float]:
        ...
        if donor_row is None:
            return (deepcopy(donor_config_flat), False, 0.0, 0.0, np.nan,)
        ...
        (new_values, uncertainty, uncertainty_raw, kernel_variance) = _select_config(
            Xraw=Xraw,
            yraw=yraw,
        ...
        return new_flat, True, float(max(0.0, uncertainty)), float(max(0.0, uncertainty_raw)), float(max(0.0, kernel_variance))
    ...
    def _apply_uncertainty_feedback(
        ...
        *,
        ...
        bridge["gp_uncertainty_raw"] = float(uncertainty_raw if guided_update else 0.0)
        bridge["gp_kernel_variance"] = float(kernel_variance if guided_update and np.isfinite(kernel_variance) else np.nan)
    ...
    def _get_new_config(self, trial: Trial, trial_to_clone: Trial) -> Tuple[Dict, Dict]:
        ...
        (nominal_flat, guided, uncertainty, uncertainty_raw, kernel_variance) = self._propose_from_gp(
