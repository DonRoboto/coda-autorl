#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CODA-SAC outer scheduler with TD-error I2O and donor-relative O2I.

Experimental variants
---------------------
- ``full``: CODA-SAC (TD-error learner context + donor-relative uncertainty O2I)
- ``i2o``:  CODA-I2O-SAC (TD-error learner context, fixed target entropy)
- ``o2i``:  CODA-O2I-SAC (no learner context, uncertainty-modulated target entropy)

Communication design
--------------------
I2O receives a bounded learner-state diagnostic computed by the experiment
callback from SAC critic TD error.  The scheduler itself remains learner-agnostic:
it consumes ``policy_update_state`` and its validity flag exactly as the PPO
instantiation did.  The callback also maintains a lineage-consistent EMA TD-error
reference, which is transferred through the scheduler bridge at checkpoint
inheritance.

O2I keeps CODA's donor-relative incremental GP uncertainty signal.  Candidate
and donor are queried under the same donor context and donor-applied SAC target
entropy:

    sigma_c(h) = sigma_obs([context_d, h,   H_target,d])
    sigma_d    = sigma_obs([context_d, h_d, H_target,d])

    U_delta(h) = clip(
        max(sigma_c(h) - sigma_d, 0)
        / (sigma_prior_candidate(h) + eps_U),
        0, 1,
    )

For SAC, the learner-side actuator is the target entropy.  The new target
entropy is generated from a fixed baseline rather than accumulated from the
donor:

    H_target,new = clip(
        H_target,0 + min(delta_max, omega_U * U_delta),
        H_target,min, H_target,max,
    )

With the recommended runner, H_target,0 = -|A| and the upper actuator bound is
-0.9|A|, so greater surrogate uncertainty requests moderately more entropy while
SAC continues to optimize its alpha parameter internally.

The outer optimizer adapts four SAC coordinates:

    train_batch_size, tau,
    optimization/actor_learning_rate,
    optimization/critic_learning_rate.

The target entropy is NOT an acquisition search coordinate; it is retained only
as an execution feature.  Actor LR, critic LR, and tau are represented in log10
space inside the GP geometry while RLlib receives executable raw values.

This file targets RLlib's old API stack because CODA needs Policy-level access
for restore-time synchronization and because SAC replay-buffer checkpointing is
part of the inherited learner state.

Repository design
-----------------
This module contains the CODA outer-loop scheduling logic: causal transition
construction, normalization, TV-GP fitting, batch diversification,
donor-relative O2I uncertainty, population exploitation, and lineage bridge
updates.

Shared SAC experiment constants are imported from ``configs.sac_config`` so the
runner and scheduler use the same scientific configuration.
"""
...
# PB2-compatible UCB constants used by CODA's acquisition layer.
UCB_BETA_OFFSET = 0.2
UCB_BETA_LOG_SCALE = 0.4
DEFAULT_ACQUISITION_RESTARTS = 10
...
def _transform_hyperparameters(
    ...
    for j, name in enumerate(names):
        is_log_scaled = (
            name in _LOG_SCALE_HYPERPARAMS
            or name.endswith("/actor_learning_rate")
            or name.endswith("/critic_learning_rate")
        )
        if is_log_scaled:
            if np.any(arr[:, j] <= 0.0):
                raise ValueError(
...
def _ucb_value(
    ...
    beta_t = UCB_BETA_OFFSET + max(0.0, np.log(UCB_BETA_LOG_SCALE * max(n_obs, 1)))
...
def _optimize_acquisition(
    ...
    candidate_model_transform,
    *,
    n_restarts: int = DEFAULT_ACQUISITION_RESTARTS,
) -> Tuple[np.ndarray, float]:
...
def _select_config(
    *,
    ...
    Historical applied target entropy remains an execution feature. For every
    ...
    the candidate's new target entropy from the fixed baseline. UCB then evaluates the
    ...
    if list(execution_feature_bounds.keys()) != [entropy_param]:
        raise ValueError(
            "CODA-SAC expects target_entropy as the sole execution feature"
        )
    ...
    baseline_entropy = float(
        np.clip(base_entropy_coeff, entropy_low, entropy_high)
    )
    ...
    reference_entropy = float(
        np.clip(reference_entropy, entropy_low, entropy_high)
    )
...
class CODASACScheduler(PopulationBasedTraining):
    """CODA-SAC with TD-error I2O and donor-relative uncertainty O2I."""
    ...
    def __init__(
        ...
        *,
        ...
        max_gp_points: int = MAX_GP_POINTS,
        entropy_param: str = "target_entropy",
        base_entropy_coeff: float = -1.0,
        o2i_uncertainty_scale: float = 0.1,
        max_entropy_increment: float = 0.1,
        actuator_bounds: Tuple[float, float] = (-1.0, -0.9),
        reward_z_clip: float = REWARD_Z_CLIP,
        ...
        self.actuator_bounds = (float(actuator_bounds[0]), float(actuator_bounds[1]))
        ...
        if not self.actuator_bounds[0] < self.actuator_bounds[1]:
            raise ValueError("actuator_bounds must satisfy low < high")
        if not (self.actuator_bounds[0] <= self.base_entropy_coeff <= self.actuator_bounds[1]):
            raise ValueError("base target entropy must lie inside actuator_bounds")
        if self.base_entropy_coeff + self.max_entropy_increment > self.actuator_bounds[1] + 1e-12:
            raise ValueError(
                "base target entropy + max increment exceeds actuator upper bound"
            )
        ...
        self._execution_feature_bounds_flat = {
            self.entropy_param: [
                float(self.actuator_bounds[0]),
                float(self.actuator_bounds[1]),
            ]
        }
    ...
    def on_trial_add(
        ...
        bridge.setdefault("lineage_td_reference_seed", None)
    ...
    def _save_trial_state(
        ...
        td_error_median = metric("sac_td_error_median")
        td_error_p95 = metric("sac_td_error_p95")
        td_error_reference_ema = metric("sac_td_error_reference_ema")
        ...
        row = {
            ...
            **dict(zip(hp_names, hp_values)),
            ...
            "sac_td_error_median": td_error_median,
            "sac_td_error_p95": td_error_p95,
            "sac_td_error_reference_ema": td_error_reference_ema,
            "is_synthetic": False,
    ...
    def _append_lineage_anchor(
        ...
        *,
        ...
        row = {
            ...
            "sac_td_error_median": donor_row.get("sac_td_error_median", np.nan),
            "sac_td_error_p95": donor_row.get("sac_td_error_p95", np.nan),
            "sac_td_error_reference_ema": donor_row.get(
                "sac_td_error_reference_ema", np.nan
            ),
            "is_synthetic": True,
            "guided_update": False,
            "gp_data_count": 0.0,
    ...
    def _apply_o2i_feedback(
        ...
        *,
        ...
        """Map donor-relative incremental completed-GP uncertainty to SAC target entropy."""
        out = deepcopy(config)
    ...
    def _set_lineage_bridge(
        ...
        *,
        ...
        if donor_row is not None:
            td_seed = _safe_float(
                donor_row.get("sac_td_error_reference_ema", np.nan), np.nan
            )
        bridge["lineage_td_reference_seed"] = (
            float(td_seed) if np.isfinite(td_seed) else None
        )
...
CODADonorRelativeIncrementalScheduler = CODASACScheduler
CODAScheduler = CODASACScheduler
CODASchedulerSAC = CODASACScheduler
W_PB2 = CODASACScheduler

__all__ = [
    "CODASACScheduler",
    "CODADonorRelativeIncrementalScheduler",
    "CODAScheduler",
    "CODASchedulerSAC",
]
